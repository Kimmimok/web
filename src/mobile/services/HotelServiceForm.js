...
function HotelServiceForm({ formData, setFormData }) {
  const [shhHeaders, setShhHeaders] = useState([]);
  const [loading, setLoading] = useState(false);
  const navigate = useNavigate();

  useEffect(() => {
    async function fetchHeaders() {
      const res = await fetch(`https://sheets.googleapis.com/v4/spreadsheets/${SHEET_ID}/values/SH_H!1:1?key=${API_KEY}`);
      const data = await res.json();
      setShhHeaders(data.values ? data.values[0] : []);
      setFormData(prev => ({
        ...prev,
        서비스ID: SHEET_ID,
        주문ID: `ORD-${Date.now()}`
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true);
    try {
      const rowData = shhHeaders.map(col => formData[col] || '');
      await fetch(
          ...
          {shhHeaders
            ...
            .map((col, idx) => (
              <div className="form-group" key={idx}>
                <label htmlFor={`shh_${col}`}>{col}</label>
                <input
                  type="text"
                  id={`shh_${col}`}
                  value={formData[col] || ''}
