...
function CruiseBookingForm({ formData, setFormData }) {
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true);
    try {
      const rowData = shrHeaders.map(col => formData[col] || '');
      await fetch(
        `https://sheets.googleapis.com/v4/spreadsheets/${SHEET_ID}/values/SH_R!A1:append?valueInputOption=USER_ENTERED&key=${API_KEY}`,
        {
          method: 'POST',
    ...
    <div className="customer-info">
      ...
      {shrHeaders.length > 0 ? (
        <form className="sheet-columns-form" onSubmit={handleSubmit}>
          {shrHeaders
            .filter(col => col !== '서비스ID' && col !== '주문ID' && col !== 'ID' && col !== '객실할인')
            .map((col, idx) => (
              <div className="form-group" key={idx}>
                <label htmlFor={`shr_${col}`}>{col}</label>
                <input
                  type="text"
                  id={`shr_${col}`}
                  value={formData[col] || ''}
