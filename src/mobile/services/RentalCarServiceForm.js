...
function RentalCarServiceForm({ formData, setFormData }) {
  const [shrcHeaders, setShrcHeaders] = useState([]);
  const [loading, setLoading] = useState(false);
  const navigate = useNavigate();

  useEffect(() => {
    async function fetchHeaders() {
      const res = await fetch(`https://sheets.googleapis.com/v4/spreadsheets/${SHEET_ID}/values/SH_RC!1:1?key=${API_KEY}`);
      const data = await res.json();
      setShrcHeaders(data.values ? data.values[0] : []);
      setFormData(prev => ({
        ...prev,
        서비스ID: SHEET_ID,
        주문ID: `ORD-${Date.now()}`
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true);
    try {
      const rowData = shrcHeaders.map(col => formData[col] || '');
      await fetch(
        `https://sheets.googleapis.com/v4/spreadsheets/${SHEET_ID}/values/SH_RC!A1:append?valueInputOption=USER_ENTERED&key=${API_KEY}`,
        {
          method: 'POST',
